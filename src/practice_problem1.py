...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        if len(self.contents+additional_contents) <= self.volume:
            ...
            for k in range(self.volume,len(self.contents)+len(additional_contents), 1):
                full = self.contents+additional_contents
                excess+=full[k]
            for k in range(0,self.volume-len(self.contents),1):
    ...
    def shrink(self, new_volume):
        ...
        if self.volume < len(self.contents):
            ...
            for k in range(self.volume):
                self.contents += old_contents[k]
            for k in range(self.volume,len(old_contents),1):
                excess += old_contents[k]
        return excess
    ...
    def double_then_shrink(self, new_volume):
        ...
        excess = len(self.double())
        excess += len(self.shrink(new_volume))
        return excess
